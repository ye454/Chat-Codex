...
9. TUI 的重要操作必须在页面正文里可见、可选、可回车执行；底部快捷键只是加速提示，不能作为唯一入口。
10. 服务启动后的运行期界面必须明确提示 `Ctrl+C` 停止服务；未验证可用前，不得提示 `Esc` / `q` 可以停止服务。
...
- `App ID`：必填。
- `App Secret`：必填，输入和展示必须隐藏，不写入 Git 或 state。
- `账号标识`：必填，是本地展示名称/实例标识，用来区分多个飞书机器人，例如 `公司机器人`、`研发助手`、`dalongxia`。它不是 secret，可以持久化。
- `飞书域`：普通用户不需要配置，默认 `feishu`。只有接入 Lark 国际版或特殊域时才作为高级项修改。

提交规则：

- 用户在最后一步按回车后，必须先校验渠道是否可通。
- 校验至少包括凭证完整、机器人身份可获取、adapter 状态为 connected。
- 校验失败时停留在飞书表单，显示中文错误，不注册为可启动渠道。
- 校验成功后注册/更新飞书渠道实例，但不进入 session 选择。
...
- 已配置渠道列表和渠道操作必须同时出现在正文里。
- 添加微信账号、添加飞书机器人不能只放在底部 `w` / `f` 快捷键里。
- 用户只使用方向键和 `Enter` 时，也必须能添加第二个、第三个渠道实例。
- `w` / `f` / `e` 等快捷键保留为加速入口，但不是唯一入口。
...
- 如果用户通过启动参数 `--cwd` / `--workdir` 指定工作目录，则本次启动使用该目录。
- TUI / Prompt CLI 中修改工作目录只保存在当前进程内，不写入本地状态。这样用户在项目 A 启动就是项目 A，在项目 B 启动就是项目 B，避免固定默认目录导致跨项目误用。
...
- 已有 session 的工作目录只展示，不提供直接修改入口。
- 如需切换工作目录，应在目标目录新建 session，并把聊天重新绑定到这个新 session。
- 一个 Codex session 当前只对应一个主要工作目录；不支持给同一个 session 添加多个工作目录。
- 用户输入不存在的目录时，必须提示确认后再创建。
- 路径可以是绝对路径，也可以是相对路径；相对路径按启动 `chat-codex` 的终端目录解析。
- 修改成功后，首页、启动确认页和运行日志页都必须展示新目录。
- 当前阶段不持久化“固定默认工作目录”。后续如果要支持固定默认目录，必须作为高级选项，并明确提示它会覆盖“在哪个项目目录启动就用哪个工作目录”的行为。
...
│   启动服务                                  │
│                                             │
│ Enter 执行  c 渠道  b 绑定  p 权限  d 目录  q 退出 │
└─────────────────────────────────────────────┘
```
...
- `Esc` / `q` 返回上一级。
- `r` 刷新状态。
- 重要动作必须是正文里的可选项，footer 快捷键不能替代正文操作。
- 首页可启动时，“启动服务”是强主操作，默认选中并用醒目颜色展示；启动后必须进入运行中面板，而不是让用户猜服务是否成功。
- 成功切换 session、权限或渠道配置后，必须显示明确结果页或底部结果提示。
- TUI 不直接读写 JSON，不直接判断 owner 冲突，不直接调用 Codex adapter；所有动作走 actions/services。
- 运行期日志界面默认提示 `Ctrl+C 停止服务`；只有在真实终端里验证过 `Esc` / `q` 能停止服务后，才能把它们写进 footer。
...
- 已完成：只保留权限配置。
- 已完成：工作目录作为单独配置项，Prompt CLI 和 TUI 都展示并可在当前进程内修改。
...
- 聊天绑定详情不提供“修改当前 session 工作目录”；换目录必须新建 session 再绑定。
- TUI 不直接读写 JSON，不直接做 owner 冲突判断。
- TUI 管理渠道页正文里必须能添加微信账号和飞书机器人，支持多渠道实例管理。
- 飞书机器人添加时账号标识必填，飞书域默认 `feishu` 且不阻塞普通用户，提交后必须校验连通性。
- 服务启动后的运行期界面必须显示 `Ctrl+C 停止服务`；未验证前不宣传 `Esc` / `q` 停止。
