...
- `codex exec --json` 模式下阶段性回复以可见事件为准，至少包含开始处理、reasoning summary、命令/工具/文件变更进度和最终回复。
...
- 命令消息不进入普通 prompt 队列，`/status`、`/cancel` 和审批命令应尽量立即处理。
...
- 记录命令执行和会话切换。
- 记录 Codex 调用状态和错误。
- `weixin codex` 常驻终端必须打印本次运行期内的微信入站消息和发回微信的 Codex 回复，方便观察真实对话流。
